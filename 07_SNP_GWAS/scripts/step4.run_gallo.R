...
option_list <- list(
  make_option(c("-s", "--snp"), type = "character", default = NULL,
              help = "Path to the SNP file", metavar = "character"),
...
if (is.null(opt$snp) | is.null(opt$qtl) | is.null(opt$gene)) {
  stop("All file paths must be provided. Use --help for more information.")
}

# Load files
QTLwindows <- load_and_validate_file(opt$snp, "txt")
...
for (c in QTL_classes) {
  tmp_file_name <- file.path(opt$output_dir, paste(c, ".jpg", sep = ""))
