...
option_list = list(
  ...
  make_option(c("-d", "--endChr"), action = "store", default = 26, type = "integer",
...
if ( startChr > endChr | startChr < 1 | endChr > 26 ) {
  stop("parameters startChr and endChr should satisfy 1 <= startChr <= endChr <= 26.")
}
...
for (chr1 in chrs) {
  ...
  # read in LRR/BAF with error handling
  dat_chr1_LRR <- tryCatch({
    fread( input = file.path( path_input, "LRR", paste0(chr1, ".tab")), header = FALSE)
  }, error = function(e) {
    warning(paste("Error: File '", file.path(path_input, "LRR", paste0(chr1, ".tab")), "' does not exist or is non-readable.", sep=""))
    return(NULL)
  })
  
  dat_chr1_BAF <- tryCatch({
    fread( input = file.path( path_input, "BAF", paste0(chr1, ".tab")), header = FALSE)
  }, error = function(e) {
    warning(paste("Error: File '", file.path(path_input, "BAF", paste0(chr1, ".tab")), "' does not exist or is non-readable.", sep=""))
    return(NULL)
  })
  
  # Skip processing if any data is missing
  if (is.null(dat_chr1_LRR) | is.null(dat_chr1_BAF)) {
    next
  }
