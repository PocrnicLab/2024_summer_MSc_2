...
for (chr1 in chrs) {
  ...
  log_file <- file.path(path_log, paste0("boundary_refine_chr", chr1, ".log"))
  err_file <- file.path(path_log, paste0("boundary_refine_chr", chr1, ".err"))

  cmd.chr1 <- paste(cmd.chr1, ">", shQuote(log_file), "2>", shQuote(err_file))

  cat("chr:", chr1, cmd.chr1, "\n")
  system(cmd.chr1)
  Sys.sleep(0.1)
}